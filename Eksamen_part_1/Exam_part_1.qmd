...
ggplot(aggr_growth_clust_long, aes(as.numeric(year), avg_growth, group_by =  country)) +
  geom_line(alpha = 0.2) +
  scale_x_continuous(breaks = seq(1960, 2022, by = 20)) +
  labs(title = "Average Growth Over the aggregated years (5) by country", x = "Year", y = "Average Growth aggregated years (5) by country")
```
...
ggplot(aggr_growth_clust_long, aes(as.numeric(year), avg_growth, group_by =  country)) +
  ...
  labs(title = "Growth over Time per 5 year by Cluster",
       x = "Year",
       y = "Average Growth aggregated years (5) by country")
```
