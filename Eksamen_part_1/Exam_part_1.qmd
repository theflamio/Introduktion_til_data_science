...
aggr_growth_long <- growth_long |>
  ...
  group_by(period, code, country, continent) |> 
  summarise(avg_growth = mean(growth))
...
## Plot avg_growth of each country as a function of year
```{r}

```


## Scaling the Data and Compute distance matrix

```{r}
# Select features 
aggr_growth_numerical <- Filter(is.numeric, aggr_growth_wide)  # Remove the country and code column before scaling

# scale the growth features
growth_scaled <- aggr_growth_numerical |>
  scale()

# Compute the distance matrix using Euclidean distance
distance_matrix <- dist(growth_scaled, method = "euclidean")
```
...
# Plot the dendrogram
hc |>
  plot(labels = aggr_growth_wide$country, main = "Dendrogram of Hierarchical Clustering", xlab = "", sub = "", cex = 0.5, hang = -1)
```


## Cutting the Dendrogram and Assigning Clusters

```{r}
k =20 # it looks like 5 cluster in the Dendrogram

# Define the number of clusters and cut the dendrogram
clusters <- cutree(hc, k) 

# Plot the dendrogram
plot(hc, labels = aggr_growth_wide$country, main = "Dendrogram of Hierarchical Clustering", xlab = "", sub = "", cex = 0.6, hang = -1)

# Highlight the k clusters on the dendrogram
