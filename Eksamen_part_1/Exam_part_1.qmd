...
```{r}
# growth_Numerical <- growth |>  # Filter(is.numeric, df) could also have been used.
#   select_if(is.numeric)

growth_Numerical <- Filter(is.numeric, growth)
...
First I want to make sure my data needs to be Standardize
```{r}
growth_Numerical2 <- scale(growth_Numerical)

distances_non_scaled <- dist(growth_Numerical, method = "euclidean")

distances_scaled <- dist(growth_Numerical2, method = "euclidean")

summary(distances_non_scaled)
summary(distances_scaled)


# Conclusion the mean moves from not scaled 15.8 to 9 that is huge so scaling is diffently needed. Data must be Standardize
...
```{r}
clustering <- hclust(distances_scaled)
clustering
```
