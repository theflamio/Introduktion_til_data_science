...
```{r, results="hide"}
# Fit a linear model
full_model <- lm(easygon ~ ., data = train)
```
...
aic_values <- data.frame(
  ...
  AIC = c(forward_aic, backward_aic, both_aic)
)
...
best_aic_index <- which.min(aic_values$AIC)
best_method <- aic_values[best_aic_index, ]
best_model <- switch(best_method$Method,
...
```{r}
summary(stepwise_full_lm_model)
```
...
pred_lm <- predict(best_model, newdata = test)
...
rmse_table <- data.frame(
  Model = c("best Linear", "Regression Tree", "Random Forest"),
  RMSE = c(rmse_lm, rmse_rpart, rmse_rf)
)
