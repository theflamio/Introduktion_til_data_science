...
```{r, fig.width=7, fig.height=7}
par(mfrow = c(2,1))
rpart.plot(tree_model, main = "Full tree", fallen.leaves = TRUE)
rpart.plot(pruned_tree_model, main = paste("Pruned tree, cp = ", optimal_cp), fallen.leaves = TRUE)
```
...
```
### 3.3.2 Visualize the tree and pruned tree

```{r}
par(mfrow = c(2,1))
rpart.plot(Classification_tree, fallen.leaves = TRUE)
rpart.plot(prunedTree, fallen.leaves = TRUE)
```
### 3.3.4 Make predictions and evaluated the tree model and Pruned tree model
...
print(confMat$table)
print(confMat$overall['Accuracy'])

pruned_predictions <- predict(prunedTree, test, type = "class")

confMat <- confusionMatrix(pruned_predictions, test$Outcome)
print("Classification tree Pruned")
print(confMat$table)
print(confMat$overall['Accuracy'])
```
...
rf_tune_model <- tuneRF(
  ...
  ntreeTry = 100,                                    # Number of trees
  stepFactor = 2,                                    # Factor to increase mtry
  improve = 0.01,                                    # Minimum improvement for tuning
  trace = TRUE                                        # Print progress
)
...
### 3.4.3 Confusion Matrix and Accuracy.
...
confMat_tuned <- confusionMatrix(predictions_tuned, test$Outcome)
print("Random forrest Tuned")
print(confMat_tuned$table)
print(confMat_tuned$overall['Accuracy'])
```
### 3.4.2 plots
...
```
### 4.3 Predict and evaluated LDA and QDA

```{r}
lda_pred <- predict(lda_model, test)
lda_classes <- lda_pred$class
conf_matrix_lda <- confusionMatrix(lda_classes, test$Outcome)

print("conf_matrix_lda")
print(conf_matrix_lda$table)
print(conf_matrix_lda$overall['Accuracy'])

qda_pred <- predict(qda_model, test)
qda_classes <- qda_pred$class
conf_matrix_qda <- confusionMatrix(qda_classes, test$Outcome)

print("conf_matrix_qda")
print(conf_matrix_qda$table)
print(conf_matrix_lda$overall['Accuracy'])
```
