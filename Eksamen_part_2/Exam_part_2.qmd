...
3. BlooPressure: Diastolic blood pressure (mm Hg)
...
zero skinThickness is not reallistic same goes for BloodPressure, BMI and DiabetesPedigreeFunction.

Seems like the data set have used 0 and 0.0 to describe missing values.

OBS Pregnancies and outcome can ofc be zero.

```{r}
dia_clean_dat <- dia_raw_dat |>
  mutate(
