...
# best trade-off between complexity (number of splits) and performance
```
...
# X-axis: This represents the complexity parameter (CP), which controls the size of the tree.Lower CP values lead to more complex trees

# Y-axis: This typically shows the cross-validated error (xerror). Lower values indicate better predictive performance.
```

### 2.4 Pick the optimal cp from previous plot and prune the tree.

```{r, results="hide"}
## from printcp cp =  0.0702 as the optimal cp because it has the lowest cross-validation error. more accurate model. 
## from printcp cp = 0.046342, as it is within one standard error of the minimum xerror "0.86815" "1-SE Rule". more simpler model.
